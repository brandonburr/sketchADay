...
#   ]
  ...
  # make 10 random rectangles and use them as keyframes for the animation
  MARGIN = 10
  MIN_SIZE = 100
  keyframe_spec = []
  for i in range(10):
    ...
    keyframe_spec.append(framerate*2)
...
for i in range(len(inset_animation)):
  ...
  v1f = "scratch/tom_eric_bw_300.jpg"
  ...
  xoffset = (IMAGE_WIDTH  - OUTPUT_IMAGE_WIDTH )/2 * (inset[0]-IMAGE_WIDTH /2)/(IMAGE_WIDTH /2) * -1
  yoffset = (IMAGE_HEIGHT - OUTPUT_IMAGE_HEIGHT)/2 * (inset[1]-IMAGE_HEIGHT/2)/(IMAGE_HEIGHT/2) * -1

  out = Image.new('RGBA', (OUTPUT_IMAGE_WIDTH, OUTPUT_IMAGE_HEIGHT), (255, 255, 255, 255))
  out.paste(i1, (0, 0))
  out.paste(inset_image, (inset[0]-xoffset, inset[1]-yoffset))
  
  # Draw bounding rect
  draw = ImageDraw.Draw(out)
  draw.rectangle([inset[0]-xoffset, inset[1]-yoffset, inset[0]-xoffset+inset[2], inset[1]-yoffset+inset[3]], fill=None, outline=(255, 255, 255))
