...
class Blob {
  constructor(x, y, numVertices) {
    ...
    this.centerVX = Utils.randomBetween(-1, 3);
    this.centerVY = Utils.randomBetween(-1, 2);
  ...
  draw() {
    ...
    for(let i = 0; i < this.dx.length - 3; i += 3) {
      bezierVertex(this.x + this.dx[i], this.y + this.dy[i], this.x + this.dx[i+1], this.y + this.dy[i+1],
										this.x + this.dx[i+2], this.y + this.dy[i+2]);
		}
		// one last bezier back towards the beginning
    bezierVertex(this.x - this.dx[1], this.y - this.dy[1], this.x - this.dx[0], this.y - this.dy[0],
      this.x, this.y);
    endShape();
...
class MySketch {
	...
	draw() {
		...
		let DELETE_SHAPE_TIME = 500;
		let NUM_SHAPES = 1;
		if(this.tick % NEW_SHAPE_TIME == 1 && this.shapes.length < NUM_SHAPES) {
			this.shapes.push(new Blob(Utils.randomBetween(0, windowWidth), Utils.randomBetween(0, windowHeight), 9));
